...
export function SeatSelector({ bus, onSeatsConfirm }: Props) {
  const { isSeatSelected, toggleSelectSeat, totalSelectedSeats, isSeatOccupied } =
    useBus();
    ...
    <div className="border-b border-gray-200 bg-white px-4 py-5 sm:px-6">
      <div className="-ml-4 -mt-2 flex flex-wrap items-center justify-between sm:flex-nowrap">
        <div className="ml-4 mt-2">
          <h3 className="text-base font-semibold leading-6 text-gray-900">{bus.name}</h3>
        </div>
      ...
      {decks.map((deck) => (
        <div key={deck} className="ml-4 mt-8 flex-shrink-0">
          <h3 className="text-base font-semibold leading-6 text-gray-900">Deck {deck}</h3>
              ...
              return (
                <div
                  ...
                  className={`${isSeatSelected(seat.id) && 'bg-green-200'} ${
                    ...
                    seatOccupied ? ' cursor-not-allowed' : ' cursor-pointer'
